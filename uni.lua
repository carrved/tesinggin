...
local hwidTable = loadstring(game:HttpGet('https://pastebin.com/raw/e6h7Ga3x'))()

print("loading vamp sneez  e")

for i,v in pairs(hwidTable) do
    if v == hwid then
        print("hwid check passed, loading...")
    end
end
...
local espLib = loadstring(game:HttpGet(('https://tsuyuri.tk/load/tabbyware/esp.lua'),true))()
espLib:Load()
...
local espLib = {
    ...
    options = {
        ...
        chams = false,
        chamsFillColor = Color3.fromRGB(201, 144, 255),
...
local library = {
    ...
    folders = {
        main = "vampsense",
        assets = "vampsense/assets",
        configs = "vampsense/configs"
    },
    shared = {
        initialized = false,
        fps = 0,
...
--local themes = {}
local theme = {
    accent = Color3.fromRGB(170, 109, 255),
...
do
    ...
    function library:New(info)
		local info = info or {}
        local name = info.name or info.Name or info.title or info.Title or "vampsense (error occured)"
        ...
        rs.RenderStepped:Connect(function()
            main_frame.Color = theme.outline
            frame_inline.Color = theme.accent
            inner_frame.Color = theme.light_contrast
            title.Size = theme.textsize
            title.Font = theme.font
            title.Color = theme.textcolor
            title.OutlineColor = theme.textborder
        end)
        --
        ...
        function window:Watermark(info)
            ...
            --
            local info = info or {}
            local watermark_name = info.name or info.Name or info.title or info.Title or string.format("vampsense || uid : %u || ping : %u || fps : %u", 1, 0, 0)
            --
            ...
            local watermark_title = utility:Create("TextLabel", {Vector2.new(2 + 6,4), watermark_outline}, {
                Text = string.format("vampsense || fps : %u || uid : %u", 35, 1),
                Size = theme.textsize,
                Font = theme.font,
                Color = theme.textcolor,
                OutlineColor = theme.textborder,
            ...
            function window.watermark:UpdateColors()
            ...
            utility:Connection(rs.RenderStepped, function(fps)
                ...
                watermark_outline.Color = theme.outline
                watermark_inline.Color = theme.inline
                watermark_frame.Color = theme.light_contrast
                watermark_accent.Color = theme.accent

                watermark_title.Size = theme.textsize
                watermark_title.Font = theme.font
                watermark_title.Color = theme.textcolor
                watermark_title.OutlineColor = theme.textborder
            end)
            --
            watermark_title.Text = string.format("[vampsense || uid : %u || ping : %i || fps : %u]", 1, tostring(library.shared.ping), library.shared.fps)
        ...
        function window:KeybindsList(info)
            ...
            --
            rs.RenderStepped:Connect(function()
                keybindslist_outline.Color = theme.outline
                keybindslist_inline.Color = theme.inline
                keybindslist_frame.Color = theme.light_contrast
                keybindslist_accent.Color = theme.accent

                keybindslist_title.Size = theme.textsize
                keybindslist_title.Font = theme.font
                keybindslist_title.Color = theme.textcolor
                keybindslist_title.OutlineColor = theme.textborder
            end)
            --
        ...
        function window:Cursor(info)
            ...
            utility:Connection(rs.RenderStepped, function()
                ...
                --
                cursor_inline.Color = theme.accent
                cursor.Color = theme.cursoroutline
            end)
            --
        ...
        local lines = {}
        --
        function window:Crosshair()
            --
            local dot = utility:new("Square",{
                Visible =  true,
                Size = Vector2.new(getgenv().Cursor.DotSize, getgenv().Cursor.DotSize),
            ...
            local connection = rs.RenderStepped:connect(function()
                if getgenv().Cursor.Enabled then
                    ...
                    dot.Size = Vector2.new(getgenv().Cursor.DotSize, getgenv().Cursor.DotSize)
                    --
    ...
    function pages:Section(info)
        ...
        --
        rs.RenderStepped:Connect(function()
            section_inline.Color = theme.inline
            section_outline.Color = theme.outline
            section_frame.Color = theme.dark_contrast
            section_accent.Color = theme.accent
        end)
        --
    ...
    function pages:MultiSection(info)
        ...
        --
        rs.RenderStepped:Connect(function()
            multiSection_accent.Color = theme.accent
        end)
        --
    ...
    function sections:Toggle(info)
        ...
        --
        rs.RenderStepped:Connect(function()
            toggle_outline.Color = theme.outline
            toggle_inline.Color = theme.inline
            toggle_frame.Color = toggle.current == true and theme.accent or theme.light_contrast
            toggle_title.Size = theme.textsize
            toggle_title.Font = theme.font
            toggle_title.Color = theme.textcolor
        end)
        --
    ...
    function sections:Slider(info)
        ...
        --
        rs.RenderStepped:Connect(function()
            slider_title.Color = theme.textcolor

            slider_outline.Color = theme.outline
            slider_inline.Color = theme.inline
            slider_frame.Color = theme.light_contrast
            slider_slide.Color = theme.accent

            slider_value.Color = theme.textcolor
        end)
        --
...
local settings_tbl = {
    ...
    ESP_TeamCheck = false,
    Chams = false,
    Chams_Color = Color3.fromRGB(171, 144, 255),
    Chams_Transparency = 0.2,
    Chams_Glow_Color = Color3.fromRGB(249, 141, 255),
    FOV = false,
    FOV_Outline = false,
}
                            ...
                            b.Transparency ~= 1 then
                                if settings_tbl.Chams then
                                    if not b:FindFirstChild("Glow") and not b:FindFirstChild("Chams") then
                                        ...
                                        glow_box.Transparency = 0.5
                                        glow_box.Material = "ForceField"
...
----local spams = {
    local vampsense = {
        '🤖 Why havent 😂 You Joined 😯 The Discord Kitten? 💀 Bad Player ⚠️',
        "Dont Worry ❌👍 I'm SMURFING‼",
        '😦 I Guess ❌ Nobody Can Read 😡',
        'I NEED ACCESS 🔑🔓 TO BBOT 🤖📃📃📃 👈 THIS THING CALLED BBOT, I NEED IT',
        "i wish i had bbort..... ",
        "I'm Just Built Different 🧱🧱🧱",
        "how does it feel to not have BoBor 🤣🤣🤣😂😂😹😹😹",
        "Jraine gangg ⛓️🩸⛓️🩸",
        "If its a depot got a bite it🥶🥶🥶",
        "Pee Pee Poo Poo Gang",
        "I LOVE 💖 YODIE GANG",
        "Wheres your radiocarthology access NN?",
        "HOw do you get ACCESS to this SCRIIPT???!!?😔😔 TELL ME NOW!!😡😡 ",
        "clipped that 🤡",
        "report him!11!!😡😡😡 omghs report!!😠😠😠😱😱😱",
...
-- // Init
do
    local title_string = "vampsense | z4xi (uid: "..library.shared.uid..") | %A, %B"
    ...
    local settings = lib:Page({name = "Settings"})
    --
    ...
    --
    local visuals_main, visuals_col = visuals:MultiSection({sections = { "Main", "Colors" }, side = "left"})
    local visuals_player = visuals:Section({name = "Players"})
    local visuals_miscellaneous = visuals:MultiSection({sections = {"Misc"}, side = "right"})
    local visuals_world = visuals:Section({name = "World", side = "right"})
    local visuals_crosshair = visuals:Section({name = "Crosshair", side = "left"})
    --
    ...
    --
    local settings_ui = settings:Section({name = "UI Settings"})
    local settings_theme = settings:Section({name = "Theme"})
    --
    ...
    --
    aimbot_br:Toggle({name = "Bullet Redirection Toggle", def = false, callback = function(v)  end})
    aimbot_br:Toggle({name = "Draw FOV", def = false, callback = function(v)  end})
    aimbot_br:Slider({name = "B.R. Hitchance", min = 0, max = 100, def = 65, suffix = "%", callback = function(v)  end})
    aimbot_br:Slider({name = "B.R. FOV", min = 0, max = 1000, def = 90, suffix = "%", callback = function(v) end})
    --
    visuals_main:Toggle({name = "Enabled", callback = function(v) settings_tbl.ESP_Enabled = v end})
    visuals_main:Toggle({name = "Draw Chams", callback = function(v) settings_tbl.Chams = v end})
    --
    visuals_col:Colorpicker({name = "Chams Fill Color", callback = function(v) settings_tbl.Chams_Color = v end})
    visuals_col:Colorpicker({name = "Chams Glow Color", callback = function(v) settings_tbl.Chams_Glow_Color = v end})
    --
    visuals_crosshair:Toggle({name = "Enabled", callback = function(v) getgenv().Cursor.Enabled = v end})
    visuals_crosshair:Slider({name = "Speed", def = 4, min = 1, max = 20, callback = function(v) getgenv().Cursor.Speed = v end})
    visuals_crosshair:Slider({name = "Radius", def = 40, min = 5, max = 100, callback = function(v) getgenv().Cursor.Radius = v end})
    visuals_crosshair:Colorpicker({name = "Color", def = Color3.fromRGB(255, 255, 255), callback = function(v) getgenv().Cursor.Color = v end})
    visuals_crosshair:Slider({name = "Thickness", def = 1, min = 0, max = 10, callback = function(v) getgenv().Cursor.Thickness = v end})
    visuals_crosshair:Toggle({name = "Outline", def = false, callback = function(v) getgenv().Cursor.Outline = v end})
    visuals_crosshair:Slider({name = "Dot Size", def = 2, min = 1, max = 10, callback = function(v) getgenv().Cursor.DotSize = v end})
    --
    visuals_world:Toggle({name = "Bloom", callback = function(v) bloom.Enabled = v end})
    visuals_world:Slider({name = "Bloom Intensity", callback = function(v) bloom.Intensity = v end})
    visuals_world:Slider({name = "Bloom Size", callback = function(v) bloom.Size = v end})
    visuals_world:Slider({name = "Bloom Threshold", decimals = 0.1, min = 0, max = 20, callback = function(v) bloom.Threshold = v end})
    --
    ...
    misc_main:Toggle({name = "Chat Spam", callback = function(v) _G.chatSpam = v end})
    misc_main:Dropdown({name = "Spam Type", options = { "vampsense", "Peroxide" }, callback = function(v) _G.chatSpamType = tostring(v) end})
    misc_main:Slider({name = "Spam Delay", min = 1, max = 5, def = 2, callback = function(v) _G.chatSpamDelay = v end})
    --
    ...
    --
    settings_theme:Colorpicker({name = "Accent Color", def = Color3.fromRGB(170, 109, 255), callback = function(v) theme.accent = v end})
    settings_theme:Colorpicker({name = "Light Contrast", def = Color3.fromRGB(30, 30, 30), callback = function(v) theme.light_contrast = v end})
    settings_theme:Colorpicker({name = "Dark Contrast", def = Color3.fromRGB(20, 20, 20), callback = function(v) theme.dark_contrast = v end})
    settings_theme:Colorpicker({name = "Outline Color", def = Color3.fromRGB(0, 0, 0), callback = function(v) theme.outline = v end})
    settings_theme:Colorpicker({name = "Inline Color", def = Color3.fromRGB(50, 50, 50), callback = function(v) theme.inline = v end})
    settings_theme:Colorpicker({name = "Text Color", def = Color3.fromRGB(255, 255, 255), callback = function(v) theme.textcolor = v end})
    settings_theme:Colorpicker({name = "Text Border", def = Color3.fromRGB(0, 0, 0), callback = function(v) theme.textborder = v end})
    settings_theme:Colorpicker({name = "Cursor Outline", def = Color3.fromRGB(10, 10, 10), callback = function(v) theme.cursoroutline = v end})
    --
    lib:Initialize()
end
--
...
while true do
    if _G.chatSpam then
        if _G.chatSpamType == "vampsense" then
            game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(moneyhook[math.random(1,moneyhook)], "All")
        elseif _G.chatSpamType == "Peroxide" then
            game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer(peroxide[math.random(1,peroxide)], "All")
        end
    end
